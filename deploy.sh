...
#  sudo apt-get update && sudo apt-get -y install sshpass
  ...
  /usr/bin/sshpass -p "$GARDEN_PASSWORD" ssh -o StrictHostKeyChecking=no $GARDEN_USER@$GARDEN_HOSTNAME "cd /home/$GARDEN_USER/workspace/GreenSense/Index/sketches/monitor/SoilMoistureSensorCalibratedSerial && /bin/pwd"
